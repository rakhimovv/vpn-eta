...
ended, and starts the next one from a menu item.
...
when you click the start item, `ifconfig` to see whether the tunnel is up, and `osascript` /
`open` to post a notification. No privileges, no network calls of its own, and it writes only
